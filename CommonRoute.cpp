...
char * find_common_route(char * hacklist, char *codelist, int *cost){
	if (hacklist == NULL || codelist == NULL || *hacklist == '\0' || *codelist == '\0'){
		*cost = 0;
		return NULL;
	}
	...
	while (hacklist[i] != '\0'){
		...
		while (codelist[j] != '\0'){
			if (hacklist[i] == codelist[j]){
				if (codelist[j] >= 'a' && codelist[j] <= 'z'){
					a->path[k] = codelist[j];
					k++;
					i++;
					j++;
				}
				else if (codelist[j] >= 'A' && codelist[j] <= 'Z'){
					a->path[k] = codelist[j];
					k++;
					i++;
					j++;
